...
public extension Reactive where Base: UIViewController {
    var willPopFromParent: Observable<Void> {
        return willMoveToParentViewController.filter { $0 == nil }.mapToVoid()
    }

    var didPopFromParent: Observable<Void> {
        return didMoveToParentViewController.filter { $0 == nil }.mapToVoid()
    }
}
